...
For this example, we will use all French Guiana but limit our query to 2021 and 2022 for a reduced output.
But we encourage users to use a larger temporal extent.
...
request <- list(
  ...
  "target" = "ERA5land_mth_FG_2021_2022.nc",
  "area" = coords
)
```
...
The result of the request is included in the package external data (`system.file("extdata", "ERA5land_mth_FG_2021_2022.nc", package = "rcontroll")`, example of use below).
...
```{r }
test_r <- suppressWarnings(rast(system.file("extdata", "ERA5land_mth_FG_2021_2022.nc", package = "rcontroll")))
...
```{r }
ggplot(filter(test, date == first(date))) +
  ...
  xlab("Longitue") +
  ylab("Latitude") +
  ggtitle(filter(test, date == first(date))$date[1])
```
...
>Because this takes time and memory, we do not run the chunk here, but invite the user to do so.
