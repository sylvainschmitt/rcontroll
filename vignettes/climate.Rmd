...
lapply(
  ...
  function(month) {
    dynamic_request(
      year = "2020",
      month = month,
      target = paste0("ERA5land_hr_FG_2020", month, ".nc")
    )
  }
)
```
...
```{r}
library(ncdf4)
p1 <- nc_open("wrf_prec_8p5.2006.nc")
p2 <- nc_open("wrf_prec_8p5.2007.nc")
...
test %>%
  ...
  gather("variable", "value", -x, -y, -date) %>%
  filter(variable %in% c("ssr", "ws", "t2m", "tp")) %>%
  ...
  geom_boxplot(fill = "lightblue", aes(group = date)) +
  geom_smooth(col = "red") +
  theme_bw() +
  xlab("") +
...
```{r }
ggplot(test, aes(x, y, fill = tp * 10^3)) +
  geom_raster() +
  theme_bw() +
  facet_wrap(~ day(date)) +
  coord_fixed() +
  viridis::scale_fill_viridis("Total\nprecipitation\n(mm)", direction = -1)
```

```{r }
ggplot(test, aes(x, y, fill = t2m - 273.15)) +
  geom_raster() +
  theme_bw() +
  facet_wrap(~ day(date)) +
  coord_fixed() +
  viridis::scale_fill_viridis("2m\ntemperature\n(°C)", direction = 1)
```

```{r }
ggplot(test, aes(x, y, fill = sqrt(u10^2 + v10^2))) +
  geom_raster() +
  theme_bw() +
  facet_wrap(~ day(date)) +
  coord_fixed() +
...
```{r }
ggplot(test, aes(x, y, fill = ssr)) +
  geom_raster() +
  theme_bw() +
  facet_wrap(~ day(date)) +
  coord_fixed() +
  viridis::scale_fill_viridis("Surface\nnet\nsolar\nradiation\n(J/m2)",
    direction = 1
  )
```
...
Simulations overview: https://cordex.org/data-access/regional-climate-change-simulations-for-cordex-domains/
Download at ESGF for South-America (SAM): https://esgf-node.ipsl.upmc.fr/search/cordex-ipsl/
