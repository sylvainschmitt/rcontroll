...
```{r fullsimG3}
rcontroll::autoplot(sim, what = "distribution", variables = c("dbh", "height", "LA", "LMA"))
```
...
gifs <- autogif(name = "dynamic",
                path = getwd(),
                variables = "height_ct",
                global = update_parameters(TROLLv3_output, nbiter = 12*100, extent_visual = 100),
                species = TROLLv3_output@inputs$species,
                climate = TROLLv3_output@inputs$climate,
                daily = TROLLv3_output@inputs$daily,
                forest = get_forest(TROLLv3_output),
...
```{r}
unlink(paste0(getwd(),"/test1"),force = TRUE)
unlink(paste0(getwd(),"/test2"),force = TRUE)
unlink(paste0(getwd(),"/teststack"),force = TRUE)
unlink(paste0(getwd(),"/dynamic"),force = TRUE)
```
