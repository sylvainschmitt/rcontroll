...
* the `forest` at start or at the end of simulation,
* the `ecosystem` temporal trajectories
* the `species` temporal trajectories
...
>Note that `autoplot` result in a `ggplot2` object that you can modify with the `+` operator and other `ggplot2` command such as `theme` used here to change the legend position.

Similarly, we can quickly plot the forest final patterns using `autoplot` and `what` defined on `forest`:
...
We can quickly plot temporal trajectories for all simulations using `autoplot` and `what` defined on ecosystem, for instance here with basal area (ba) and aboveground biomass (agb):
...
Then we extracted the previous simulation parameters to run the new one in similar conditions, we only change the time length to 12 iterations.
...
For that we updated the simulation iteration index by adding the number realized in the previous simulation.
...
list("TROLLv3_output" = TROLLv3_output@ecosystem,
  ...
  theme_bw() +
  xlab("Time (years)") +
  ylab(expression(Abovergound~biomass~(AGB~MgC~ha^{-1})))
```

>Note how the simulation AGB start at the end of `TROLLv3_output`'s temporal trajectory.  
