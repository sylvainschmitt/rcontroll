...
void Tree::Update()
{
  ...
  if (t_age)
  {
    ...
    if (_NDD)
      ...
      if (_LA_regulation == 0)
        death = int(gsl_rng_uniform(gslrng) + DeathRate(t_dbh, npp_neg));
      else
        death = int(gsl_rng_uniform(gslrng) + DeathRate(t_dbh, t_carbon_storage)); // newIM 2021: directly use the t_carbon_storage variable instead of NPPneg
#endif
    if (death)
      Death();
    else
