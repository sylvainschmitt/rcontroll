...
void Species::Init()
{
  ...
  if (_distdisperse)
  {
    ...
    if (s_DispSynd == 0)
    { // s_Dispsynd = 0 indicates animal-dispersed species which is the baseline level, so no coefficient
...
#ifdef LCP_alternative
#else
float Species::CalcVcmaxm()
{                                                                                                                                                              // added v.3.1, IM, needed to compute s_LCP in Species::Init (needed for filter seedling recruitement depending on light availability)
...
void CompareParameters(N &parameter_value, N parameter_min, N parameter_max, N parameter_default, string &parameter_name, vector<string> &parameter_names, vector<string> &parameter_values, bool quiet)
{
  ...
  if (parameter_value_emp != parameter_default)
  {
    ...
    if (!equal)
      Rcout << "Supplied parameter " << parameter_name << " does not correspond to calculated value at precision " << precision << endl;
  }
}
...
int Tree::BirthFromInventory(int site, vector<string> &parameter_names, vector<string> &parameter_values, int &nb_speciesrandom)
{
  ...
  if (t_dbh > 0.0)
  {
    ...
    if ((t_leaflifespan == 0.0) | (t_lambda_young == 0.0) | (t_lambda_mature == 0.0) | (t_lambda_old == 0.0))
      CalcLeafLifespan(); // if the Kikuzawa model is used, the leaflifespan will be modified by a random error term, which may considerably affect tree performance if the value is recomputed
    ...
    if (_LA_regulation > 0)
    {
      ...
      if (t_carbon_storage == -1.0)
        t_carbon_storage = carbon_storage_max * 0.5; // We here initialize trees with half their maximum storage
...
void Tree::Water_availability()
{
  ...
  if (t_LA > 0.0)
  { // TO BE CHECKED: what happens to a tree with leafarea=0 in the following timesteps ?
    // Tree root biomass
...
float Tree::DeathRate(float dbh, int nppneg, float phi_root)
{
  ...
  if (phi_root < (S[t_sp_lab].s_phi_lethal))
    dr += 1.0 / timestep;
  if (iter == int(nbiter - 1) && _OUTPUT_extended == 1)
    output_extended[4] << iter << "\t" << t_wsg << "\t" << dbh << "\t" << basal << "\t" << dr << "\n";
  return dr * timestep;
}
#else
...
#ifdef WATER
  if (iter == 2)
    OutputTreeStandard(output_water[1]);
  if (iter == int(nbiter / 2))
    OutputTreeStandard(output_water[2]);
  if (iter == int(nbiter - 1))
    OutputTreeStandard(output_water[3]);

  if (t_site == 4)
    OutputTreeStandard(output_water[4]);
  if (t_site == 10380)
    OutputTreeStandard(output_water[5]);
  if (t_site == 100950)
    OutputTreeStandard(output_water[6]);
  if (t_site == 12090)
    OutputTreeStandard(output_water[7]);
  if (t_site == 120090)
    OutputTreeStandard(output_water[8]);
  if (t_site == 150667)
    OutputTreeStandard(output_water[9]);
#endif
}
...
void Tree::CalcRespGPP()
{
  if (t_LA > 0.0)
  {
    ...
    for (int h = crown_above_top; h >= h_stop; h--)
    {
      ...
      if (isnan(t_transpiration) || t_transpiration < 0.0)
        Rcout << "Problem at site: " << t_site << " transpiration: " << t_transpiration << " leafarea_layer: " << leafarea_layer << " GPP: " << t_GPP << " PPFD: " << PPFD << " VPD: " << VPD << " T: " << Tmp << " t_WSF_A: " << t_WSF_A << " t_WSF: " << t_WSF << endl
              << endl;
#else
...
void Tree::Death()
{
  ...
  {
    if (iter == 2)
      output_extended[3] << iter << "\t" << S[t_sp_lab].s_name << "\t" << t_age << "\t" << t_dbh << "\t" << t_height << "\n";
    if (iter == int(nbiter / 2))
      output_extended[3] << iter << "\t" << S[t_sp_lab].s_name << "\t" << t_age << "\t" << t_dbh << "\t" << t_height << "\n";
    if (iter == int(nbiter - 1))
      output_extended[3] << iter << "\t" << S[t_sp_lab].s_name << "\t" << t_age << "\t" << t_dbh << "\t" << t_height << "\n";
...
void Tree::DisperseSeed()
{
  if (t_dbh >= t_dbhmature)
  {
    ...
    for (int i = 0; i < nbs; i++)
    {
      ...
      if (_torus)
      { // Implementing a torus (if the dispersal exceed the limits of the simulation, it goes to the other side, so that sites in the extremes also potentially receive seeds from all sides
    ...
    std::string output_file)
{
  ...
  if (!output_info)
    Rcerr << "ERROR with info file" << endl;
  ...
  Rcout << "Number of species: " << nbspp << endl
        << endl;

  if (_GPPcrown == 1)
    Rcout << "Activated Module: FastGPP" << endl;
  if (_BASICTREEFALL == 1)
    Rcout << "Activated Module: BASICTREEFALL" << endl;
  if (_NDD == 1)
    Rcout << "Activated Module: NDD" << endl;
  if (_SEEDTRADEOFF == 1)
    Rcout << "Activated Module: SEEDTRADEOFF" << endl;
  if (_FromInventory == 1)
    Rcout << "Activated Module: FromInventory" << endl;
  if (_OUTPUT_extended == 1)
    Rcout << "Activated Module: OUTPUT_extended" << endl;
  if (_OUTPUT_extended == 1 && extent_visual > 0)
    Rcout << "Activated visualization output." << endl;
  if (_OUTPUT_pointcloud == 1)
    Rcout << "Activated Module: Point cloud output (simplified ALS simulation)" << endl; // v.3.1.6
  if (_fecundity == 1)
    Rcout << "Activated Module: fecundity" << endl;
  if (_Rrecruit == 1)
    Rcout << "Activated Module: recruit rate" << endl;
  if (_distdisperse == 1)
    Rcout << "Activated Module: dispersal distance" << endl;
  if (_torus == 1)
    Rcout << "Activated Module: torus" << endl;
  if (_MinLAImax == 1)
  ...
  if (isnumeric)
  {
    ...
    { // built in precision check, quite tolerant (1%)
      if (numeric < parameter_min)
        ...
        parameter = numeric;
      if (!quiet)
        Rcout << parameter_name << ": " << parameter << endl;
    }
    else
    {
  ...
  if (In)
  {
    ...
    else
    {
      ...
      for (int mrow = 0; mrow < 3; mrow++)
        Rcout << gsl_matrix_get(mcov_N_P_LMA, mrow, 0) << "\t" << gsl_matrix_get(mcov_N_P_LMA, mrow, 1) << "\t" << gsl_matrix_get(mcov_N_P_LMA, mrow, 2) << endl;
...
void ReadInputSoil()
{
  ...
  if (InSoil)
  {
    ...
    if (NULL == (layer_depth = new float[nblayers_soil]))
      Rcerr << "!!! Mem_Alloc layer_depth" << endl;
    ...
    if (NULL == (Sat_SWC = new float[nblayers_soil]))
      Rcerr << "!!! Mem_Alloc Sat_SW" << endl;
    if (NULL == (Max_SWC = new float[nblayers_soil]))
      Rcerr << "!!! Mem_Alloc Max_SW" << endl;
    ...
    if (NULL == (phi_e = new float[nblayers_soil]))
      Rcerr << "!!! Mem_Alloc phi_e" << endl;
    if (NULL == (b = new float[nblayers_soil]))
      Rcerr << "!!! Mem_Alloc b" << endl;
    for (int l = 0; l < nblayers_soil; l++)
    {
...
void InitialiseLookUpLAImax()
{
  ...
  for (int spp = 1; spp < nbspp + 1; spp++)
  {
    ...
    for (int dev = 0; dev < 10000; dev++)
    {
      ...
      if (pseudotree.t_LAImax < minLAImax_A)
        minLAImax_A = pseudotree.t_LAImax;
      if (pseudotree.t_LAImax > maxLAImax_A)
        maxLAImax_A = pseudotree.t_LAImax;
...
void InitialiseLookUpTables()
{
  ...
  if (NULL == (LookUp_KmT = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_KmT" << endl;
  if (NULL == (LookUp_GammaT = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_GammaT" << endl;
  if (NULL == (LookUp_VcmaxT = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_VcmaxT" << endl;
  if (NULL == (LookUp_JmaxT = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_JmaxT" << endl;
  if (NULL == (LookUp_Rday = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_Rday" << endl;
  if (NULL == (LookUp_Rstem = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_Rstem" << endl;
  if (NULL == (LookUp_Rnight = new float[nbTbins]))
    Rcerr << "!!! Mem_Alloc LookUp_Rnight" << endl;
...
#ifdef Audrey
  if (NULL == (MinLAImax = new float[nbspp + 1]))
    cerr << "!!! Mem_Alloc\n";
  if (NULL == (MaxLAImax = new float[nbspp + 1]))
    cerr << "!!! Mem_Alloc\n";
  ...
  if (InInventory)
  {
    ...
    else
    {
      if ((flag_col == 1) | (flag_row == 1))
        Rcout << "WARNING! At least one coordinate column (col/row) is missing, random coordinates chosen" << endl;
      if (flag_species == 1)
        Rcout << "WARNING! No species column provided, random species chosen" << endl;
      ...
      if (nb_parameterlines > 0)
      {
        ...
        if (sites_shuffled_index == sites)
          Rcout << "WARNING: shuffle index: " << sites_shuffled_index << " is equal site number. This means that random placement of trees has stopped early as the algorithm has run out of grid cells to place trees. This may indicate that there were more trees than grid cells." << endl;
      }
      else
      {
...
void AllocMem()
{
...
#endif
  if (NULL == (SPECIES_GERM = new unsigned int[nbspp + 1]))
    Rcerr << "!!! Mem_Alloc\n"; // Field for democratic seed germination

#ifdef Audrey
  if (_Rrecruit)
  {
    if (NULL == (prob_recruit = new double[nbspp + 1]))
      cerr << "!!! Mem_Alloc\n";
  ...
  if (NULL == (SPECIES_SEEDS = new int *[sites]))
    Rcerr << "!!! Mem_Alloc\n"; // Field of seeds
  for (int site = 0; site < sites; site++)
  {                                                         // For each processor, we define a stripe above (labelled 0) and a stripe below (1). Each stripe is SBORD in width.
  ...
  if (NULL == (LAI_DCELL = new float[nbdcells]))
    Rcerr << "!!! Mem_Alloc\n";
...
void UpdateField()
{
  ...
  for (int site = 0; site < sites; site++)
    LAI_DCELL[site_DCELL[site]] += LAI3D[0][site + SBORD];
  for (int d = 0; d < nbdcells; d++)
    LAI_DCELL[d] *= i_sites_per_dcell;
#endif
  ...
  for (int site = 0; site < sites; site++)
    T[site].Water_uptake(); // Update of Transpiration: tree water uptake, each tree will deplete soil water content through its transpiration
  ...
  for (int d = 0; d < nbdcells; d++)
  {
    ...
    if (Evaporation[d] < 0 || isnan(Evaporation[d]) || (SWC3D[0][d] - Min_SWC[0]) < 0)
      Rcout << "evaporation=" << Evaporation[d] << " and e=" << e << " and SWC3D[0][d]=" << SWC3D[0][d] << " and Min_SWC[0]=" << Min_SWC[0] << "and SWC3D[0][d]-Min_SWC[0]=" << SWC3D[0][d] - Min_SWC[0] << endl;
  ...
  if ((col >= 0) && (col < cols))
  {
    if ((row >= 0) && (row < rows))
    {
      ...
      if (_SEEDTRADEOFF)
        ...
        SPECIES_SEEDS[site][spp] = 1; // If s_Seed[site] = 0, site is not occupied, if s_Seed[site] > 1, s_Seed[site] is the age of the youngest seed
    ...
    if (T[site].t_age == 0.0)
    {
      ...
      { // ... and then randomly select one of these species
        if (_Rrecruit)
        {
          ...
          if (spp <= nbspp)
            SPECIES_GERM[spp] = 0; // reinitilized SPECIES_GERM with zero, now that we have found which species is to be recruited (spp)
        }
        else
        {
...
void TriggerTreefallSecondary()
{
  ...
  for (int site = 0; site < sites; site++)
  {
    if (T[site].t_age)
    {
      ...
      if (2.0 * T[site].t_hurt * gsl_rng_uniform(gslrng) > height_threshold)
      { // check whether tree dies: probability of death is 1.0-0.5*t_height/t_hurt, so gslrng <= 1.0 - 0.5 * t_height/t_hurt, or gslrng > 0.5 * t_height/t_hurt; modified in v.2.5: probability of death is 1.0 - 0.5*t_height/(t_mult_height * t_hurt), so the larger the height deviation (more slender), the higher the risk of being thrown by another tree
        if (p_tfsecondary > gsl_rng_uniform(gslrng))
  ...
  if (Abu10_prev > 1)
  {
    ...
    for (int r = row_start; r < row_end; r++)
    {
      for (int c = col_start; c < col_end; c++)
      {
        ...
        float error;
        if (prob < 0.95)
          error = gsl_ran_gaussian(gslrng, sd1);
        else
          error = gsl_ran_gaussian(gslrng, sd2);
...
void CloseOutputs()
{
  ...
  if (_OUTPUT_pointcloud == 1)
    output_pointcloud.close(); // added in v.3.1.7 to properly close LAS file
