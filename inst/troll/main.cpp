...
float R; //!< Global variable: distance beyond which negative density dependence (NDD) effect is not accounted anymore
...
void UpdateField() {
    ...
    if(_NDD){
        ...
        for(int site=0;site<sites;site++) {
            ...
            for(int col=max(0,int(col0-R));col<=min(cols-1,int(col0+R));col++) {
                for(int row=max(0,int(row0-R));row<=min(rows-1,int(row0+R));row++) {  // loop over the neighbourhood 
                    int xx=col0-col;
                    int yy=row0-row;
