...
#' @rdname get_chm
#' @export
setGeneric('get_chm', function(sim,  method = "smoothed", ...) {return(standardGeneric('get_chm'))})
...
setMethod("get_chm", "trollsim", function(sim, method = "smoothed", ...) {
  ...
  if(method == "filled")
    chm <- lapply(chm, focal, w, fun = fill.na)
  if(method == "smoothed")
    chm <- lapply(chm, focal, w, fun = mean, na.rm = TRUE)
  
  # changes names
  chm <- lapply(chm, function(x){names(x) <- "canopy_height" ; return(x)})
