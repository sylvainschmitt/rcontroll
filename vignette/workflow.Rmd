...
author:
  - name: Sylvain Schmitt
    affiliation: 
    - UMR Biodiversité Gènes et Communautés, Université de Bordeaux
    - UMR Écologie des Forêts de Guyane, Kourou
    email: sylvain.schmitt@agroparistech.fr
  - name: Nino Page
    affiliation: UMR Écologie des Forêts de Guyane, Kourou
date: '`r Sys.Date()`'
package: RconTroll
abstract: >
  Instructions and example of a workflow to run simulations with both TROLL and RconTroll.
output:
...
Within R:
```{r}
build(src = file.path(path, "main_2.3.1.cpp"), # C++ code
      app = "troll", # name of the app
      path = path, # path to compile
      optimisation = "-Ofast") # optimisation
```
...
plot(single_test, what = "agb", ggplot2 = T, plotly = T)
```

```{r, fig.cap="Aboveground biomass along time in the multiple simulations test."}
plot(multiple_test, what = "agb", ggplot2 = T)
```
