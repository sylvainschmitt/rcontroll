...
Generate_LHS_Autocalib <- function(nsim = 100, 
  ...
  nparam <- dim(na.exclude(paramLHS))[1]
  ...
  X <- pse::LHScorcorr(X,COR = corrmat,eps = 0.001*sqrt(nsim), echo = TRUE,maxIt = 1E10)
  X <- rbind(as.matrix(X), matrix(rep(t(X), nreplicat -1), ncol= nparam+1 , byrow=TRUE))
  
  Random_rows_X <-sample(nrow(X)) 
  X <- X[Random_rows_X,]
...
SumVar <- function(i,
  ...
  LSAgb <- MCtotal %>% dplyr::filter(simulation == i,iter >= 12*Nyears-12*Nsampling, agb != "NaN")%>% mutate(LogAgb = log(agb+1)) %>% dplyr::select(LogAgb)
  LSAgb[LSAgb == Inf] <- NA
  LSAgb[LSAgb == NaN] <- NA
  LSAgb[LSAgb == -Inf] <- NA
  ...
  LSAbu1 <- MCtotal %>% dplyr::filter(simulation == i,iter >= 12*Nyears-12*Nsampling, sum10 != "NaN")%>% mutate(LogAbu1 = log(sum1+1)) %>% dplyr::select(LogAbu1)
  LSAbu1[LSAbu1 == Inf] <- NA
  LSAbu1[LSAbu1 == NaN] <- NA
  LSAbu1[LSAbu1 == -Inf] <- NA
  if (length(na.exclude(LSAbu1)) != 0 & sum(na.exclude(LSAbu1)) != 0 ) {
    TSAbu1 <- ts(LSAbu1,frequency = 12,start = c(Nyears - Nsampling,1))
    TDAbu1 <- decompose(TSAbu1)
    MCAbu1 <- coda::mcmc(na.exclude(TDAbu1$x - TDAbu1$seasonal))
    
    EstParam <- c(EstParam,as.numeric(summary(MCAbu1)$statistics[1]))
    EstParam <- c(EstParam,as.numeric(summary(MCAbu1)$statistics[2]))
...
extractVar <- function(stacktmp,
  ...
  close(pb)
  stopCluster(cl)
  results <- matrix(data = stack_res,ncol = 24,byrow = FALSE)
...
autocalibGP <- function(Generated_parameters,
  ...
  if (GPComputation) {
    if (!is.null(NiterHetGP)) {
      ...
      XGPCor <- Generated_parameters$XGP[1:dim(Y)[1],] %>% as_tibble() %>% filter(!(row_number() %in% NA_values_index)) %>%  as.matrix()
      ...
      prdataMeanAbu1 <- hetGP::find_reps(XGPCor, Ycor[,4], rescale = FALSE, normalize = FALSE)
      prdataSdAbu1 <- hetGP::find_reps(XGPCor, Ycor[,5], rescale = FALSE, normalize = FALSE)
      prdataMeanAbu10 <- hetGP::find_reps(XGPCor, Ycor[,6], rescale = FALSE, normalize = FALSE)
      prdataSdAbu10 <- hetGP::find_reps(XGPCor, Ycor[,7], rescale = FALSE, normalize = FALSE)
      prdataMeanAbu30 <- hetGP::find_reps(XGPCor, Ycor[,8], rescale = FALSE, normalize = FALSE)
      prdataSdAbu30 <- hetGP::find_reps(XGPCor, Ycor[,9], rescale = FALSE, normalize = FALSE)
      prdataMeanGpp <- hetGP::find_reps(XGPCor, Ycor[,10], rescale = FALSE, normalize = FALSE)
      prdataSdGpp <- hetGP::find_reps(XGPCor, Ycor[,11], rescale = FALSE, normalize = FALSE)
      prdataMeanNpp <- hetGP::find_reps(XGPCor, Ycor[,12], rescale = FALSE, normalize = FALSE)
      prdataSdNpp <- hetGP::find_reps(XGPCor, Ycor[,13], rescale = FALSE, normalize = FALSE)
      prdataMeanBa10 <- hetGP::find_reps(XGPCor, Ycor[,14], rescale = FALSE, normalize = FALSE)
      prdataSdBa10 <- hetGP::find_reps(XGPCor, Ycor[,15], rescale = FALSE, normalize = FALSE)
      prdataMeanHill <- hetGP::find_reps(XGPCor, Ycor[,16], rescale = FALSE, normalize = FALSE)
      prdataSdHill <- hetGP::find_reps(XGPCor, Ycor[,17], rescale = FALSE, normalize = FALSE)
      prdataMeanRao <- hetGP::find_reps(XGPCor, Ycor[,18], rescale = FALSE, normalize = FALSE)
      prdataSdRao <- hetGP::find_reps(XGPCor, Ycor[,19], rescale = FALSE, normalize = FALSE)
      prdataHmean <- hetGP::find_reps(XGPCor, Ycor[,20], rescale = FALSE, normalize = FALSE)
      prdataLambda1ter <- hetGP::find_reps(XGPCor, Ycor[,21], rescale = FALSE, normalize = FALSE)
      prdataVolECMP <- hetGP::find_reps(XGPCor, Ycor[,22], rescale = FALSE, normalize = FALSE)
      prdataVolECMPS <- hetGP::find_reps(XGPCor, Ycor[,23], rescale = FALSE, normalize = FALSE)
      prdataGmDBH <- hetGP::find_reps(XGPCor, Ycor[,24], rescale = FALSE, normalize = FALSE)
      ...
      mod.MeanAbu1 <- hetGP::mleHetGP(X = list(X0 = prdataMeanAbu1$X0,Z0 = prdataMeanAbu1$Z0,mult=prdataMeanAbu1$mult), Z = prdataMeanAbu1$Z,covtype = "Gaussian")
      mod.SdAbu1 <- hetGP::mleHetGP(X = list(X0 = prdataSdAbu1$X0,Z0 = prdataSdAbu1$Z0,mult=prdataSdAbu1$mult), Z = prdataSdAbu1$Z,covtype = "Gaussian")
      ...
      h.MeanAbu1 <- rep(0, NiterHetGP)
      h.SdAbu1 <- rep(0, NiterHetGP)
      ...
      for (j in initj:NiterHetGP) {
        ...
        h.MeanAbu1[j] <- hetGP::horizon(mod.MeanAbu1)
        h.SdAbu1[j] <- hetGP::horizon(mod.SdAbu1)
        ...
        opt.MeanAbu1 <- hetGP::IMSPE_optim(mod.MeanAbu1, h = h.MeanAbu1[j],ncores = ncores)
        opt.SdAbu1 <- hetGP::IMSPE_optim(mod.SdAbu1, h = h.SdAbu1[j], ncores = ncores)
        ...
        Xnew <- rbind(opt.RateDBH$par,
                      ...
                      opt.MeanAbu1$par,opt.SdAbu1$par,
        ...
        if (ncores_sim < 24) {
          ...
          message("Computing augmented maximin LHS")
          XRd<- lhs::augmentLHS(XRd, ncores - 24)
          XRdNew <- XRd[(dim(XRd)[1]-(ncores - 25)):(dim(XRd)[1]),]
          message("Computing augmented correlated LHS")
          XRdNew <- pse::LHScorcorr(XRdNew,COR = Generated_parameters$corrmat,eps = 0.001*sqrt(ncores - 22), echo = TRUE,maxIt = 1E10)
        ...
        for (blocki in (New_generated_parameters$initk/ncores_sim +1):max(nrep$ID_iter)) {
          ...
          if (inherits(sim_stack,"try-error")) {
            ...
            for (blocki_temp in 1:max(nrep_tmp$ID_iter)) {
              ...
              if (inherits(sim_stack_i,"try-error")) {
                Ynew <- matrix(rep(NA, times = 24*ncores),nrow = ncores)
      ...
      mod.MeanAgb <- hetGP::mleHetGP(X = list(X0 = prdataMeanAgb$X0,Z0 = prdataMeanAgb$Z0,mult=prdataMeanAgb$mult), Z = prdataMeanAgb$Z,covtype = "Gaussian")
      mod.SdAgb <- hetGP::mleHetGP(X = list(X0 = prdatSdAgb$X0,Z0 = prdatSdAgb$Z0,mult=prdatSdAgb$mult), Z = prdatSdAgb$Z,covtype = "Gaussian")
      mod.MeanAbu1 <- hetGP::mleHetGP(X = list(X0 = prdataMeanAbu1$X0,Z0 = prdataMeanAbu1$Z0,mult=prdataMeanAbu10$mult), Z = prdataMeanAbu1$Z,covtype = "Gaussian")
      mod.SdAbu1 <- hetGP::mleHetGP(X = list(X0 = prdataSdAbu1$X0,Z0 = prdataSdAbu1$Z0,mult=prdataSdAbu1$mult), Z = prdataSdAbu1$Z,covtype = "Gaussian")
      mod.MeanAbu10 <- hetGP::mleHetGP(X = list(X0 = prdataMeanAbu10$X0,Z0 = prdataMeanAbu10$Z0,mult=prdataMeanAbu10$mult), Z = prdataMeanAbu10$Z,covtype = "Gaussian")
      mod.SdAbu10 <- hetGP::mleHetGP(X = list(X0 = prdataSdAbu10$X0,Z0 = prdataSdAbu10$Z0,mult=prdataSdAbu10$mult), Z = prdataSdAbu10$Z,covtype = "Gaussian")
      mod.MeanAbu30 <- hetGP::mleHetGP(X = list(X0 = prdataMeanAbu30$X0,Z0 = prdataMeanAbu30$Z0,mult=prdataMeanAbu30$mult), Z = prdataMeanAbu30$Z,covtype = "Gaussian")
